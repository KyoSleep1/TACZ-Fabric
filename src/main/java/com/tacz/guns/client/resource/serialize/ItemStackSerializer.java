...
public class ItemStackSerializer implements JsonDeserializer<ItemStack> {
    ...
    private static Item getItem(String itemName) {
        Identifier itemKey = Identifier.of(itemName);
    ...
    private static ItemStack getItemStack(JsonObject json) {
        ...
        if (json.has("nbt")) {
            ...
            return ItemStack.fromNbt(BuiltinRegistries.createWrapperLookup(), tmp)
                    .orElse(new ItemStack(item, JsonHelper.getInt(json, "count", 1)));
        } else {
